...
async function request(path, options = {}) {
  ...
  if (!response.ok) {
    if (response.status === 401 && !path.startsWith("/auth/")) {
      ...
      localStorage.removeItem("lavapro_token");
...
const mapPedidoToFrontend = (order) => {
  ...
  const status =
    stateMap[(order.estado_actual || "").toLowerCase()] || order.estado_actual;
  ...
  if (order.pagos && order.pagos.length > 0) {
    const approvedPayment = order.pagos.find(
      (p) => p.estado.toLowerCase() === "approved",
    );
    if (approvedPayment) {
  ...
  const trazabilidad = sortedTracing.map((he) => {
    ...
    else if (normEst === "en proceso")
      displayTitle = "Lavado Iniciado (En Proceso)";
    else if (normEst === "listo")
      displayTitle = "Empaquetado y Colgado (Listo)";
  ...
  return {
    ...
    cliente: order.usuario
      ? `${order.usuario.nombre} ${order.usuario.apellido || ""}`.trim()
      : "Desconocido",
...
export const api = {
  ...
  async getPedidos(userRole, clientName = "") {
    ...
    if (userRole === "cliente" && clientName) {
      return mapped.filter(
        (p) =>
          p.cliente.toLowerCase() === clientName.toLowerCase() ||
          p.cliente.toLowerCase().includes(clientName.toLowerCase()),
      );
    }
    return mapped;
  },
  ...
  async updatePedido(id, data) {
    ...
    if (updateBody.status) {
      ...
      updateBody.status =
        stateMapInv[updateBody.status.toLowerCase()] || updateBody.status;
    }
  ...
  async addServicio(data) {
    ...
    const unitObj = units.find(
      (u) => u.nombre.toLowerCase() === data.unidad.toLowerCase(),
    );
    ...
    if (!unitObj || !modObj) {
      throw new Error(
        `Unidad '${data.unidad}' o Modalidad '${data.modalidad}' no válida`,
      );
    }
